...
#define VIRTUAL_DISK_PREFIX "vd"
#define NVME_PREFIX_LENGTH 4
#define SCSI_PREFIX_LENGTH 2
#define VIRTUAL_DISK_LENGTH 2
...
static int get_disk_type_by_name(char *disk_name_addr){
    if (strlen(disk_name_addr) > NVME_PREFIX_LENGTH
        ...
        return SCSI;
    } else if (strlen(disk_name_addr) > VIRTUAL_DISK_LENGTH
        && !strncmp(disk_name_addr, VIRTUAL_DISK_PREFIX, VIRTUAL_DISK_LENGTH)){
        return SCSI;
    }
    else {
        log_format("unknown disk %s\n", disk_name_addr);
        return UNKOWN;
    }
};
...
static unsigned long class_kset_parse_klist(unsigned long class_addr){
    unsigned long class_kset_addr;
    unsigned long sp_subsys_offset;
    unsigned long sp_class_val;
    unsigned long kset_list_addr;
    unsigned long cur_list;
    unsigned long kobj_addr, kset_addr, sp_addr;
    unsigned long kobj_entry_offset, kset_obj_offset;

    class_kset_addr = symbol_value("class_kset");
    kobj_entry_offset = MEMBER_OFFSET("kobject", "entry");
    kset_obj_offset = MEMBER_OFFSET("kset", "kobj");
    sp_subsys_offset = MEMBER_OFFSET("subsys_private", "subsys");

    kset_list_addr = class_kset_addr + MEMBER_OFFSET("kset", "list");
    list_for_each(cur_list, kset_list_addr){
        kobj_addr = cur_list - kobj_entry_offset;
        kset_addr = kobj_addr - kset_obj_offset;
        sp_addr = kset_addr - sp_subsys_offset;
        get_field(sp_addr, "subsys_private", "class", &sp_class_val);
        if (class_addr == sp_class_val){
            return sp_addr + MEMBER_OFFSET("subsys_private", "klist_devices");
        }
    }
    log_format("parse klist device failed, return NULL\n");
    return 0;
}

static unsigned long get_class_klist_list_addr(unsigned long class_addr){
    ...
    if ((subsys_private_offset = MEMBER_OFFSET("class", "p")) >= 0){
        get_field(class_addr, "class", "p", &subsys_private_addr);
        if( (klist_devices_offset = MEMBER_OFFSET("subsys_private","klist_devices")) >= 0){
            ...
            log_format("struct subsys_private do not have klist_devices, try member class_devices");
            klist_devices_offset = MEMBER_OFFSET("subsys_private", "class_devices");
            klist_devices_addr = subsys_private_addr + klist_devices_offset;
        }
    } else {
        if ( (klist_devices_offset = MEMBER_OFFSET("class", "klist_devices")) >=0 ){
            ...
            klist_devices_addr = class_kset_parse_klist(class_addr);
        }
    }
...
static unsigned long get_device_elevator_name(unsigned long request_queue_addr){
    ...
    if (MEMBER_EXISTS("elevator_queue", "elevator_type")){
        ...
        if (0x01 == MEMBER_TYPE("elevator_type", "elevator_name"))
            get_field(elevator_type_addr, "elevator_type", "elevator_name", &elevator_name_addr);
        else
             elevator_name_addr = elevator_type_addr + MEMBER_OFFSET("elevator_type", "elevator_name");
    } else {
...
static unsigned long mq_tags_parse_sbit_tag(unsigned long mq_tags_addr, char *tags_name)
{
    unsigned long addr = 0;

    if (0x01 == MEMBER_TYPE("blk_mq_tags", tags_name))
        get_field(mq_tags_addr, "blk_mq_tags", tags_name, &addr);
    else
        addr = mq_tags_addr + MEMBER_OFFSET("blk_mq_tags", tags_name);
...
static void mq_tags_parse_request_info(struct gendisk_info *gendisk, unsigned long tags_addr)
{
    ...
    unsigned long bitmap_tags = mq_tags_parse_sbit_tag(tags_addr, "bitmap_tags");
    unsigned long breserved_tags_addr = mq_tags_parse_sbit_tag(tags_addr, "breserved_tags");
...
static unsigned long xanode_parse_hctx(unsigned long xa_node, unsigned long idx)
{
    unsigned long hctx;
    unsigned long node;
    unsigned char shift;
    ...
    get_field(xa_node, "xa_node", "shift", &shift);
    if ((idx >> shift) > 63){
        log_format("idx bigger than xarray");
        hctx = 0;
        return hctx;
    }

    offset = (idx >>shift) & 63;
    readmem(xa_node + offset * sizeof(void *), KVADDR,
            &node, sizeof(unsigned long),
            "unsigned long", FAULT_ON_ERROR);
    if(!node || !(node & 2)){
        return node;
    }
    return xanode_parse_hctx(node, idx);
}

static unsigned long xarray_parse_hctx(unsigned long xarr, unsigned long idx)
{
    unsigned long ret;
    unsigned long xa_head;

    get_field(xarr, "xarray", "xa_head", &xa_head);
    if (!(xa_head & 2)){
        return idx == 0 ? xa_head : 0;
    }else{
        ret = xanode_parse_hctx(xa_head & ~3, idx);
    }
    return ret;
}

static unsigned long request_queue_parse_hctx(unsigned long request_queue, unsigned long idx)
{
    unsigned long hctx_addr;
    unsigned long hctx_table;
    unsigned long queue_hw_ctx_addr;

    if (MEMBER_EXISTS("request_queue", "queue_hw_ctx")){
        get_field(request_queue, "request_queue", "queue_hw_ctx", &queue_hw_ctx_addr);
        readmem(queue_hw_ctx_addr + idx * sizeof(unsigned long),
        KVADDR, &hctx_addr, sizeof(unsigned long),
        "unsigned long", FAULT_ON_ERROR);
    }else if (MEMBER_EXISTS("request_queue", "hctx_table")){
        hctx_table = request_queue + MEMBER_OFFSET("request_queue", "hctx_table");
        hctx_addr = xarray_parse_hctx(hctx_table, idx);
    }
...
static int gendisk_parse_request_info(struct gendisk_info *gendisk){
    ...
    for (idx = 0; idx <nr_hw_queues; idx++){
        hctx = request_queue_parse_hctx(gendisk->request_queue_addr, idx);
...
static unsigned long device_parse_gendisk_addr(unsigned long device_addr)
{
    unsigned long gendisk;
    unsigned long block_device;

    if (STRUCT_EXISTS("hd_struct")){
        block_device = device_addr - MEMBER_OFFSET("hd_struct", "__dev");
        gendisk = block_device - MEMBER_OFFSET("gendisk", "part0");
    }else{
        block_device = device_addr - MEMBER_OFFSET("block_device", "bd_device");
        get_field(block_device, "block_device", "bd_disk", &gendisk);
    }
    return gendisk;

}

static unsigned long klist_device_parse_device_addr(unsigned long node_addr)
{
    unsigned long device_addr = 0;
    unsigned long knode_class_offset = 0;
    unsigned long knode_node_offset = MEMBER_OFFSET("klist_node", "n_node");

    if ((knode_class_offset = MEMBER_OFFSET("device", "knode_class")) == -1){
        knode_class_offset = MEMBER_OFFSET("device_private", "knode_class");
        device_addr = node_addr - knode_node_offset - knode_class_offset;
        get_field(device_addr, "device_private", "device", &device_addr);
    }else{
        device_addr = node_addr - knode_node_offset - knode_class_offset;
    }
...
static int is_vaild_nvme_name(const char *name) {
    const char *p_pos = strchr(name, 'p');
    if (p_pos == NULL) {
        return TRUE;
    }

    if (*(p_pos + 1) == '\0') {
        //disk name like nvme0n1p, have same gendisk addr with nvme0n1
        return FALSE;
    }

    return isdigit(*(p_pos + 1)) ? FALSE : TRUE;
}

static int is_valid_disk_name(unsigned long device_addr)
{
    int len;
    int ret;
    unsigned long kobj_addr;
    unsigned long kobj_name_addr;
    char disk_name[MAX_DESC_LENGTH];

    kobj_addr = device_addr + MEMBER_OFFSET("device", "kobj");
    get_field(kobj_addr, "kobject", "name", &kobj_name_addr);

    get_string(kobj_name_addr, disk_name, MAX_DESC_LENGTH);

    ret = TRUE;
    if(strncmp(disk_name, SCSI_PREFIX, SCSI_PREFIX_LENGTH) == 0
            || strncmp(disk_name, VIRTUAL_DISK_PREFIX, VIRTUAL_DISK_LENGTH) == 0){
        len = strlen(disk_name);
        if (len > 0 && isdigit(disk_name[len - 1])){
            //disk name like sda1, vda1, have same gendisk addr with sda, vda
            ret = FALSE;
        }
    }else if (strncmp(disk_name, NVME_PREFIX, NVME_PREFIX_LENGTH) == 0){
        ret = is_vaild_nvme_name(disk_name);
    }else{
        log_format("not sd, vd and nvme disk\n");
        ret = FALSE;
    }
...
static int block_class_parse_gendisk_info(struct block_class_info *block_class)
{
    ...
    list_for_each(list_head_next, list_head_base_addr){
        device_addr = klist_device_parse_device_addr(list_head_next);
        if (is_valid_disk_name(device_addr)){
            gendisk_addr = device_parse_gendisk_addr(device_addr);
            tmp = calloc(1, sizeof(*tmp));
...
static void gendisk_scsi_info_dump(struct gendisk_info *gendisk, int with_request)
{
    ...
    fprintf(fp, "gendisk       : 0x%-24lx    |    scsi_device : 0x%lx\n",
            gendisk->gendisk_addr,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->sdev_addr : 0);
    fprintf(fp, "request_queue : 0x%-24lx    |    H:C:T:L     : %d:%d:%d:%d\n",
            gendisk->request_queue_addr,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->host_no : 0,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->sdev_channel : 0,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->sdev_id : 0,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->sdev_lun : 0);
    fprintf(fp, "elevator_name : %-24s       |    Vendor/Model : %s\n",
            gendisk->elevator_type,
            gendisk->scsi_device_info ? gendisk->scsi_device_info->vendor : "--NA--");
    fprintf(fp, "-----------------------------------------------------------"
        "-----------------------------------------------------------\n");
    if (with_request){
...
static void scsi_device_parse_channel_info(struct scsi_device_info *scsi_device){
    ...
    get_field(scsi_device->sdev_addr, "scsi_device", "host", &scsi_host_addr);
...
static void scsi_host_parse_hostt_info(struct scsi_host_info *scsi_host)
{
    unsigned long hostt_module_addr;
    unsigned long module_version_addr;
...
static void scsi_host_parse_base_info(struct scsi_host_info *scsi_host){
    unsigned long host_name_addr = 0;
    unsigned long host_busy_addr = 0;
    unsigned long host_block_addr = 0;
    unsigned long host_busy_offset = 0;

    if ((host_busy_offset =  MEMBER_OFFSET("Scsi_Host", "host_busy")) != -1)
    ...
    scsi_host_parse_hostt_info(scsi_host);

    if (host_busy_addr != 0)
        get_atomic_value(host_busy_addr, &scsi_host->host_busy);
    else
        scsi_host->host_busy = -1;
...
static int shost_class_parse_scsi_host_info(struct shost_class_info *shost_class_info){
    ...
    unsigned long scsi_host_addr = 0;
